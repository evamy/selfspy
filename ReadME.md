...
Selfspy is only tested with Python 3.8.6 and has a few dependencies on other Python libraries that need to be satisfied. These are documented in the requirements.txt file. 
...
Report issues here:
https://github.com/evamy/selfspy/issues
...
Selfspy mainly creates two files `selfspy` and `selfstats` in `/usr/bin/` (or as in this project `/usr/local/bin/`) folder. To uninstall it from your system which may be due to any reason (trying to reinstall and/or testing things and the code keeps giving an error of `ImportError: from selfspy.activity_store import ActivityStore`).