...
def make_time_string(dates, clock):
    ...
    if len(dates) > 3:
        print('Max three arguments to date ' + str(dates))
        sys.exit(1)
    ...
    try:
        ...
        print('Malformed date ' + str(dates))
        sys.exit(1)
    ...
    if clock:
        try:
            ...
            print('Malformed clock ' + str(clock))
            sys.exit(1)
...
class Selfstats:
    ...
    def show_rows(self):
        ...
        for row in fkeys:
            rows += 1
            print(row.id, end=' ')
            print(row.started, end=' ')
            print(pretty_seconds((row.created_at - row.started).total_seconds()), end=' ')
            print(row.process.name, end=' ')
            print('"%s"' % row.window.title, end=' ')
            print(row.nrkeys, end=' ')
            if self.args['showtext']:
                if self.need_humanreadable:
    ...
    def show_summary(self):
        ...
        if self.need_activity:
            ...
            if act:
                ...
                act = 0
            print('Total time active: '+ str(pretty_seconds(act)))
            print()

        if self.args['clicks']:
            print('Mouse clicks:')
            for key, name in BUTTON_MAP:
                print(str(self.summary.get(key, 0)) + ' ' + name)
            print()
        ...
        if self.args['pkeys']:
            ...
            for name, data in pdata:
                print(name + ' ' + str(data.get('keystrokes', 0)))
            print()
